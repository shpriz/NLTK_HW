...
df_train = pd.read_csv('data/train_tweets.csv')
df_test = pd.read_csv('data/test_tweets.csv')
...
def my_worker(gpu_id):
    with cuda.Device(gpu_id):
        # 1. Удаление @user из всех твитов с помощью паттерна "@[\w]*"
        pattern_for_username = "@[\w]*"
        df['tweet'] = df['tweet'].progress_apply(lambda x: delete_pattern(x, pattern_for_username))

        # 2. Изменение регистра твитов на нижний с помощью .lower()
        df['tweet'] = df['tweet'].progress_apply(lambda x: x.lower())

        # 3
        df['tweet'] = df['tweet'].progress_apply(lambda x: replace_words(x, apostrophe_dict))

        # 4. Замена сокращений на их полные формы с использованием short_word_dict.
        df['tweet'] = df['tweet'].progress_apply(lambda x: replace_words(x, short_word_dict))

        # 5. Замена эмотиконов (пример: ":)" = "happy") на пробелы с использованием emoticon_dict.
        df['tweet'] = df['tweet'].progress_apply(lambda x: replace_words(x, emoticon_dict))

        # 6. Замена пунктуации на пробелы с использованием re.sub() и паттерна r'[^\w\s]'.
        pattern_for_punctuation = r'[^\w\s]'
        df['tweet'] = df['tweet'].progress_apply(lambda x: delete_pattern(x, pattern_for_punctuation))

        # 7. Замена спец. символов на пробелы с использованием re.sub() и паттерна r'[^a-zA-Z0-9]'.
        pattern_for_special_symb = r'[^a-zA-Z0-9]'
        df['tweet'] = df['tweet'].progress_apply(lambda x: delete_pattern(x, pattern_for_special_symb))
        ...
        min_len = 1
        df['tweet'] = df['tweet'].progress_apply(lambda x: delete_short_words(x, min_len))

        # 10. Деление твитов на токены с помощью nltk.tokenize.word_tokenize с созданием нового столбца 'tweet_token'.
        df['tweet_token'] = df['tweet'].progress_apply(lambda x: tknz.word_tokenize(x))
        df['tweet_token_filtered'] = df['tweet_token'].progress_apply(lambda x: delete_stop_words(x, stop_words))
        df['tweet_stemmed'] = df['tweet_token_filtered'].progress_apply(lambda x: compare_stemmer(x, stemmer))
        df['tweet_lemmatized'] = df['tweet_token_filtered'].progress_apply(lambda x: compare_lemmatizer(x, lemmatizer))
...
def main():
    ...
    num_gpu = cuda.runtime.getDeviceCount()
    gpu_ids = range(num_gpu)
    print(f'gpu_ids={num_gpu}')

    with multiprocessing.Pool(len(gpu_ids)) as pool:
        pool.map(my_worker, gpu_ids)

    # 14. Сохранение результата предобработки в pickle-файл.
    with open('data/NLP_HW_1_results.pickle', 'wb') as f:
        pickle.dump(df, f)
